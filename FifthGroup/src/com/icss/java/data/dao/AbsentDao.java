...
public class AbsentDao {
	...
	public boolean loginUser(String userID, String userPassword){
		...
		// 通过?占位符的方式设置参数值
		String sql = " select * from absent where staffID=? and userPassword=?";
	 ...
	 * @return
	 */
	public List<AbsentEntity> queryAll(){
		// 用户集合对象
		List<AbsentEntity> list = new ArrayList<AbsentEntity>();
...
//				System.out.println(rs.getString(1)+"-"+rs.getString(2)+"-"+rs.getDate(3)
	 ...
	 * @return
	 */
	public boolean addUser(AbsentEntity de){
		boolean flag = false;
		//1. 管理者 Connection
		Connection conn = DBUtile.getConn();
		...
		try {
			...
			pstmt.setDate(3, de.getAbsenttime());
			pstmt.setString(4, de.getReason());
	 ...
	 * @return
	 */
	public boolean updateUser(AbsentEntity de){
		boolean flag = false;
		//1. 管理者 Connection
		Connection conn = DBUtile.getConn();

		//2. 任务 sql语句
		String sql = "update absent set Name=?,absenttime=?,reason=? where staffID=?";
		try {
			//3. 执行者 PreparedStatement   管理者把任务给执行者
			PreparedStatement pstmt = conn.prepareStatement(sql);
			...
			pstmt.setDate(2, de.getAbsenttime());
			pstmt.setString(3, de.getReason());
			pstmt.setString(4, de.getStaffID());
	...
	public boolean deleteUser(String staffID){
		...
		//2. 任务 sql语句
		String sql = "delete from absent where staffID=?";
		try {
			//3. 执行者 PreparedStatement   管理者把任务给执行者
			PreparedStatement pstmt = conn.prepareStatement(sql);
