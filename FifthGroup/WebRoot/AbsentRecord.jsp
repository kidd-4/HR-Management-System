...
<%@ page import="com.icss.java.data.dao.AbsentEntity"%>
<%
...
<script type="text/javascript">
	 function goDelete(sId){
		...
		if(confirm("是否删除用户"+sId+"?")){
			// 跳转到删除Action
			location.href='doDeleteAbsent?staffID='+sId;
		}
	}
...
<%
	//　获得请求中绑定的数据信息--用户集合 
	List<AbsentEntity> list = (List<AbsentEntity>)request.getAttribute("stuList");
%>
  ...
  <TR>
    ...
    <TD vAlign=top width="100%" bgColor=#ffffff>
      <TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
        <TR>
          <TD class=manageHead>当前位置：管理首页 &gt;考勤管理&gt;缺勤记录</TD></TR>
        <TR>
      ...
      align=center border=0>
            ...
            cellSpacing=1 cellPadding=2 rules=all border=0>
              ...
              style="FONT-WEIGHT: bold; FONT-STYLE: normal; BACKGROUND-COLOR: #eeeeee; TEXT-DECORATION: none">
                ...
                <%
                		...
                		AbsentEntity la = list.get(i);
                %>
                <form action="AbsentUpdate.jsp" method="post">                                   
              <TR 
              ...
              <td>
							...
							<td>
								<%= la.getAbsenttime() %>
								<input type="hidden" name="absenttime" value="<%= la.getAbsenttime() %>" />
							</td>
							<td>
								<%= la.getReason() %>
        ...
        <TR>
          <TD align=right height=25><button onclick="location.href='AbsentAdd.jsp'">添加缺勤记录</button>   &nbsp;&nbsp; </TD></TR>
        <TR>
          <TD><SPAN id=pagelink>
            <DIV 
