...
<%@ page import="com.icss.java.data.dao.LateEntity"%>
<%
...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD id=Head1><TITLE>迟到记录</TITLE>
...
<script type="text/javascript">
	function goDelete(staffID){ 
		if(confirm("是否删除?"+staffID+"?")){
			location.href='doLateDelete?staffID='+staffID;		
		}
	}
...
<META content="MSHTML 6.00.2900.3492" name=GENERATOR></HEAD>
  ...
  <%List<LateEntity> list = (List<LateEntity>)request.getAttribute("LateList");%>
  ...
  <TR>
    ...
    <TD vAlign=top width="100%" bgColor=#ffffff>
      <TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
        <TR>
          <TD class=manageHead>当前位置：管理首页 &gt;考勤管理&gt;迟到记录</TD></TR>
        <TR>
      ...
      align=center border=0>
              ...
              style="FONT-WEIGHT: bold; FONT-STYLE: normal; BACKGROUND-COLOR: #eeeeee; TEXT-DECORATION: none">
                ...
                <%
                	for(int i=0;i<list.size();i++){
                		LateEntity la = list.get(i);
                %>
                <form action="UpdateLate.jsp" method="post">                                   
              <TR 
              style="FONT-WEIGHT: normal; FONT-STYLE: normal; BACKGROUND-COLOR: white; TEXT-DECORATION: none">
               <td>
						    <%= la.getstaffID() %>
						    <input type="hidden" name="staffID" value="<%= la.getstaffID() %>" />
						</td> 
						<td>
						    <%= la.getName() %>
						    <input type="hidden" name="Name" value="<%= la.getName() %>" />
						</td>
						<td>
						    <%= la.getarrivetime() %>
							<input type="hidden" name="arrivetime" value="<%= la.getarrivetime() %>" />
						</td>
						<td>
								<%= la.getreason() %>
								<input type="hidden" name="reason" value="<%= la.getreason() %>" />
							</td>
				
                <TD>
                <center>
                <input type="submit" value="修改" />&nbsp;&nbsp;
	          <button type="button" onclick="goDelete('<%= la.getstaffID() %>')">删除</button>
	            </center>
                </TR>
                </form>
        ...
        <TR>
          <TD align=right height=25><button onClick="location.href='AddLate.jsp'">添加迟到记录</button>   &nbsp;&nbsp; </TD></TR>
        <TR>
          <TD><SPAN id=pagelink>
            <DIV 
