...
<%@ page import="com.icss.java.data.dao.StaffDocumentEntity"%>
<%
...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD id=Head1><TITLE>人员档案管理</TITLE>
...
<script type="text/javascript">
	 function goDelete(docID){
		...
		if(confirm("是否删除用户"+docID+"?")){
			...
			// 跳转到删除Action
			location.href='doDelete2?docID='+docID;	
...
<%
	//　获得请求中绑定的数据信息--用户集合 
	List<StaffDocumentEntity> list = (List<StaffDocumentEntity>)request.getAttribute("stuList");
%>
  ...
  <TR>
    ...
    <TD vAlign=top width="100%" bgColor=#ffffff>
      <TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
        <TR>
          <TD class=manageHead>当前位置：管理首页 &gt;人员档案&gt;档案信息</TD></TR>
        <TR>
      ...
      align=center border=0>
            ...
            cellSpacing=1 cellPadding=2 rules=all border=0>
              ...
              <th>档案编号</th>
                ...
                <%
                		...
                		StaffDocumentEntity se = list.get(i);
                %>
                <form action="updateStaffDocument.jsp" method="post">                                   
              <TR 
              style="FONT-WEIGHT: normal; FONT-STYLE: normal; BACKGROUND-COLOR: white; TEXT-DECORATION: none">
               <td>
							...
							<td>
								<%= se.getDocTime() %>
								<input type="hidden" name="pos" value="<%= se.getDocTime() %>" />
							</td>
							<td>
								<%= se.getDocCon() %>
								<input type="hidden" name="dep" value="<%= se.getDocCon() %>" />
        ...
        <TR>
          <TD align=right height=25><button onclick="location.href='registStaffDocument.jsp'">添加档案</button>   &nbsp;&nbsp; </TD></TR>
        <TR>
          <TD><SPAN id=pagelink>
            <DIV 
